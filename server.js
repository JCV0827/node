...
app.use(cors({
  origin: 'http://node-iota-livid.vercel.app',
  credentials: true,
...
app.use('/uploads', (req, res, next) => {
  res.header('Access-Control-Allow-Origin', 'http://node-iota-livid.vercel.app');
...
app.post('/register', upload.single('profilePicture'), (req, res) => {
  ...
  connection.query(userQuery, [name, email, password, organization, profilePicture, null], (err, results) => {
    ...
    connection.query(deviceQuery, [userId, device, cpu, gpu, ram, capacity, motherboard, psu], (err) => {
      ...
      const profileImageUrl = profilePicture ? `https://node-iota-livid.vercel.app/uploads/${profilePicture}` : null;
...
app.get('/user', authenticateToken, (req, res) => {
  ...
  connection.query(userQuery, [userId], (err, userResults) => {
    ...
    if (userResults.length > 0) {
      ...
      const profileImageUrl = user.profile_image ? `https://node-iota-livid.vercel.app/uploads/${user.profile_image}` : null;
  ...
  try {
    ...
    connection.query(deviceIdQuery, [userId], (err, deviceIdResults) => {
      ...
      connection.query(userQuery, [currentDeviceId], async (err, userResults) => {
        ...
        const cpuResponse = await fetch(`https://node-iota-livid.vercel.app/cpu_usage?model=${cpu}`);
        const gpuResponse = await fetch(`https://node-iota-livid.vercel.app/gpu_usage?model=${gpu}`);
        const ramResponse = await fetch(`https://node-iota-livid.vercel.app/ram_usage?model=${ram}`);
  ...
  try {
    ...
    connection.query(deviceIdQuery, [userId], (err, deviceIdResults) => {
      ...
      connection.query(userQuery, [currentDeviceId], async (err, userResults) => {
        ...
        const cpuResponse = await fetch(`https://node-iota-livid.vercel.app/cpum_usage?model=${cpu}`);
        const gpuResponse = await fetch(`https://node-iota-livid.vercel.app/gpum_usage?model=${gpu}`);
        const ramResponse = await fetch(`https://node-iota-livid.vercel.app/ram_usage?model=${ram}`);
...
app.post('/send-reset-email', async (req, res) => {
  ...
  connection.query(query, [email], (err, results) => {
      ...
      const resetLink = `http://node-iota-livid.vercel.app/reset-password?token=${resetToken}`;
      const mailOptions = {
  ...
  connection.query(query, [projectId], (err, results) => {
    ...
    const members = results.map(member => ({
      ...
      profile_image: member.profile_image 
        ? `https://node-iota-livid.vercel.app/uploads/${member.profile_image}`
        : null
    }));
